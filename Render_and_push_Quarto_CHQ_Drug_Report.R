...
tryCatch({
  ...
  # Filter status: only look for changes matching the expected output files
  # status$file often uses forward slashes, ensure output_files also uses them
  normalized_output_files <- gsub("\\\\", "/", output_files) 
  files_to_add <- intersect(normalized_output_files, status$file[status$status != "ignored"]) 
  ...
  # If no specific output files changed, consider adding all changed files
  if(length(files_to_add) == 0 && nrow(status) > 0) {
    log_message("No specific output files changed. Checking all changed files...")
    # Only consider unstaged or untracked files (not ignored)
    changed_files <- status$file[status$status != "ignored"]
    log_message(paste("All changed files:", paste(changed_files, collapse=", ")))
    
    # Optionally add only files in specific directories (_site/ or specific extensions)
    site_files <- grep("^_site/", changed_files, value = TRUE)
    if(length(site_files) > 0) {
      log_message(paste("Found changes in _site directory:", paste(site_files, collapse=", ")))
      files_to_add <- site_files
    }
  }
  ...
  if (length(files_to_add) > 0) {
    ...
    # Optional: Check if staging area is actually different before committing
    # diff_staged <- git_diff_staged(repo = repo_dir)
    # if(nrow(diff_staged) > 0) { ... commit ... } else { log_message("No changes staged to commit.") }
    ...
    log_message("No changes detected in specified files to commit.")
  }
